...
func (api *apiFeature) theResponseShouldBe(arg1 string) error {
	...
	if res.Translation != arg1 {
		return fmt.Errorf("translation should be set %s", arg1)
	}
...
func InitailizeTestSuite(sc *godog.TestSuiteContext) {
	var err error
...
func InitailizeScenario(ctx *godog.ScenarioContext) {
	client := resty.New()
	api := &apiFeature{
		client: client,
	}
	...
	ctx.Before(func(ctx context.Context, sc *godog.Scenario) (context.Context, error) {
		...
		cfg.DatabasePort = database.GetPort("6379")
